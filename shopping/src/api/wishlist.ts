...
export default (app: Application) => {
    ...
    app.put("/addToWishlist", async (req, res, next) => {
        try {
            ...
            const newCart = await wishlistService.addItemToWishlist(userId, itemDTO);
    ...
    app.put("/removeFromWishlist", async (req, res, next) => {
        try {
            ...
            const newCart = await wishlistService.removeItemFromWishlist(userId, productVariantId);
            return res.status(200).json(newCart);
        } catch (error) {
            next(error);
            return;
